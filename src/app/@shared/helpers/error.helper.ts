...
export class ErrorHelper {
  ...
  handleGenericError(err) {
    ...
    if (err.status === 0) {
      // couldn't establish a connection to the backed
      ...
      if (error.status === 404 && error.name === 'INVALID_ENDPOINT') {
        ...
        this.router.navigate(['/auth/login'], { queryParams: { return: this.router.url } })
          .then(() => {
            ...
            if (hasBeenLogged) {
              toast = { type: 'warning', title: translate('TOKEN_EXP_TITLE'), body: translate('TOKEN_EXP_MSG') };
            } else {
              toast = { type: 'error', title: translate('NOT_LOGGED_TITLE'), body: translate('NOT_LOGGED_MSG') };
            }
